...
GO
/****** Object:  View [{{!SQL.SCHEMA}}].[sienaCurrencyPair]    Script Date: 04/12/2021 18:16:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE VIEW [{{!SQL.SCHEMA}}].[sienaCurrencyPair]
AS
SELECT        {{!SQL.SOURCE}}.CurrencyPair.CodeMajorCurrencyIsoCode, {{!SQL.SOURCE}}.CurrencyPair.CodeMinorCurrencyIsoCode, {{!SQL.SOURCE}}.CurrencyPair.ReciprocalActive, 
...
WHERE        ({{!SQL.SOURCE}}.CurrencyPair.InternalDeleted IS NULL) AND ({{!SQL.SOURCE}}.Currency.Active = 1) AND (Currency_1.Active = 1) AND ({{!SQL.SOURCE}}.CurrencyPair.ReciprocalActive = 1)
GO
